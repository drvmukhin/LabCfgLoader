...
Dim nCommand, vCommand, bRollBack, nRollBack, bConnect
Dim Platform
Dim objTab_L
...
'--------------------------------------------------------------------------------
	...
	For nSession = 4 to crt.Arguments.Count - 1
	    Do
			...
			On Error Goto 0
			bConnect = True
			Call TrDebug_No_Date ("START LOADING CONFIGURATION TO " & UCase(strSessionL), "", objDebug, MAX_LEN, 3, 1)		
			...
			'-------------------------------------------------------
			objTab_L.Screen.Send "show version" & chr(13)
			nResult = objTab_L.Screen.WaitForStrings (vModels, 2)
			 Select Case nResult
				Case 1, 2, 3 
				  nRe = 1
				  vRe = Array("re0","re1")
			  ...
			  Case Else
				  ...
				  vRe = Array("re0")
			 End Select
			If Int(nResult) > 0 Then strPlatform = vModels(nResult - 1) Else strPlatform = "N/A"
			Call  TrDebug_No_Date ("Host " & strSessionL & " was qualified as: " & UCase(strPlatform), "", objDebug, MAX_LEN, 1, nInfo)
			objTab_L.Screen.Send chr(13)
			objTab_L.Screen.WaitForString "@" & strHostL & "> "		
			'---------------------------------------------
			...
			'---------------------------------------------
				...
				objTab_L.Screen.WaitForString "@" & strHostL & "#"
				objTab_L.Screen.Send "save " & GLOBAL_CFG & chr(13)
				objTab_L.Screen.WaitForString "@" & strHostL & "#"
				objTab_L.Screen.Send "exit" & chr(13)
				objTab_L.Screen.WaitForString "@" & strHostL & "#"
			'---------------------------------------------
			'   Save current re0/re1 groups
			'---------------------------------------------
			For Each strRe in vRe
				objTab_L.Screen.Send "edit groups " & strRe & chr(13)
				objTab_L.Screen.WaitForString "@" & strHostL & "#"
				objTab_L.Screen.Send "show" & chr(13)
				objTab_L.Screen.WaitForString "@" & strHostL & "#"		
				objTab_L.Screen.Send "save " & RE_CFG & strRe & ".conf" & chr(13)
				objTab_L.Screen.WaitForString "@" & strHostL & "#"
				objTab_L.Screen.Send "exit" & chr(13)
				objTab_L.Screen.WaitForString "@" & strHostL & "#"
			Next
...
'				objTab_L.Screen.WaitForString "@" & strHostL & ">"
		...
		'			crt.quit
			...
			objTab_L.Screen.WaitForString "ftp>"
			'---------------------------------------------
			'   FTP TRANSFER CFG FILE
			'---------------------------------------------
			objTab_L.Screen.Send "get " & SourceFolder & "/" & strConfigFileL & " " & strConfigFileL & chr(13)
			If Not objTab_L.Screen.WaitForString ("Successfully transferred", 60) Then
				Call  TrDebug_No_Date ("FTP Uploading configuration file:", "FAILED", objDebug, MAX_LEN, 1, 1)
				objTab_L.Session.Disconnect
				bConnect = False
				Exit Do		
			Else
				Call  TrDebug_No_Date ("FTP Uploading configuration file: ", "OK", objDebug, MAX_LEN, 1, 1)  
			End If
			objTab_L.Screen.WaitForString "ftp>"
			...
			objTab_L.Screen.WaitForString "@" & strHostL & "#"
			'-------------------------------------------------------
			' DELETE Protocols and Other Services
			'-------------------------------------------------------
			Call  TrDebug_No_Date ("Cleaning old configuration:", "......IN PROGRESS", objDebug, MAX_LEN, 1, nInfo)   
			objTab_L.Screen.Send "delete access" & chr(13)
			objTab_L.Screen.WaitForString "@" & strHostL & "# "
			objTab_L.Screen.Send "delete services" & chr(13)
			objTab_L.Screen.WaitForString "@" & strHostL & "# "
			objTab_L.Screen.Send "delete routing-instances" & chr(13)
			objTab_L.Screen.WaitForString "@" & strHostL & "# "
			objTab_L.Screen.Send "delete protocols" & chr(13)
			objTab_L.Screen.WaitForString "@" & strHostL & "# "
			...
			Select Case nResult
				Case 0
					Call  TrDebug_No_Date ("Committing configuration on " & strSessionL, "TIME OUT", objDebug, MAX_LEN, 1, 1)   
					Exit Sub
				Case 1 
					Call  TrDebug_No_Date ("Committing configuration on " & strSessionL, "ERROR 1", objDebug, MAX_LEN, 1, 1)   
					bRollback = True
					nRollBack = "1"				
					Exit Do
				Case 2 
					Call  TrDebug_No_Date ("Committing configuration on " & strSessionL, "ERROR 2", objDebug, MAX_LEN, 1, 1)   
					bRollback = True
					nRollBack = "1"				
					Exit Do
				Case Else
					Call  TrDebug_No_Date ("Cleaning configuration: ", "OK", objDebug, MAX_LEN, 1, 1)   
			End Select	
'			objTab_L.Screen.WaitForString "@" & strHostL & "# "		
			'---------------------------------------------
			...
			nResult = objTab_L.Screen.WaitForStrings(vLoadComplete, 60)
			Select Case nResult
				Case 0 
				  Call  TrDebug_No_Date ("Loading Configuration from File: ", "TIME OUT", objDebug, MAX_LEN, 1, 1)
				  Exit Sub
				Case 1, 2
					objTab_L.Screen.WaitForString "@" & strHostL & "# "
					bRollBack = True
					nRollBack = "1"
					Call TrDebug_No_Date("Loading Configuration from File: " ,"ERROR" , objDebug, MAX_LEN, 1, 1)
					Exit Do
				Case Else
				   Call  TrDebug_No_Date ("Loading Configuration from File: ", "OK", objDebug, MAX_LEN, 1, 1)   
			End Select
			'----------------------------------------------------------
			' UPDATE CONFIG FOR group global
			'----------------------------------------------------------
			objTab_L.Screen.Send "edit groups global" & chr(13)
			objTab_L.Screen.WaitForString "@" & strHostL & "# "
			objTab_L.Screen.Send "load replace " & GLOBAL_CFG & chr(13)
			nResult = objTab_L.Screen.WaitForStrings(vLoadComplete, 60)
			Select Case nResult
				Case 0 
					Call TrDebug_No_Date("Restoring Group Configuration ""global"":" ,"TIME OUT", objDebug, MAX_LEN, 1, 1)
					Exit Sub
				Case 1, 2
					objTab_L.Screen.WaitForString "@" & strHostL & "# "
					bRollBack = True
					nRollBack = "1"
					Call TrDebug_No_Date("Restoring Group Configuration ""global"":", "ERROR" , objDebug, MAX_LEN, 1, 1)
					Exit Do
				Case Else
				    Call TrDebug_No_Date("Restoring Group Configuration ""global"":", "OK" , objDebug, MAX_LEN, 1, 1)
			End Select
			objTab_L.Screen.Send "exit" & chr(13)
			objTab_L.Screen.WaitForString "@" & strHostL & "# "
			...
			objTab_L.Screen.WaitForString "@" & strHostL & "#"
			'----------------------------------------------------------
			' UPDATE config for group re0/re1
			'----------------------------------------------------------
			For Each strRe in vRe
				objTab_L.Screen.Send "edit groups " & strRe & chr(13)
				objTab_L.Screen.WaitForString "@" & strHostL & "# "
				objTab_L.Screen.Send "load replace " & RE_CFG & strRe & ".conf" & chr(13)
				nResult = objTab_L.Screen.WaitForStrings(vLoadComplete, 60)
				Select Case nResult
					Case 0 
						Call TrDebug_No_Date("Restoring Configuration Group """ & strRe & """:", "TIME OUT", objDebug, MAX_LEN, 1, 1)
						Exit Sub
					Case 1, 2
						objTab_L.Screen.WaitForString "@" & strHostL & "# "
						bRollBack = True
						nRollBack = "1"
						Call TrDebug_No_Date("Restoring Configuration Group """ & strRe & """:" ,"FAILD" , objDebug, MAX_LEN, 1, 1)
						Exit Do
					Case Else
						Call TrDebug_No_Date("Restoring Configuration Group """ & strRe & """:" ,"OK" , objDebug, MAX_LEN, 1, 1)
				End Select
				objTab_L.Screen.Send "exit" & chr(13)
				objTab_L.Screen.WaitForString "@" & strHostL & "# "
				objTab_L.Screen.Send "set apply-groups " & strRe & chr(13)	
				objTab_L.Screen.WaitForString "@" & strHostL & "#"				
			Next	
			'---------------------------------------------
			'   COMMIT FINAL
			'---------------------------------------------	
			bRollback = False
			Call  TrDebug_No_Date ("Committing new configuration:", "......IN PROGRESS", objDebug, MAX_LEN, 1, nInfo)   
			objTab_L.Screen.Send "commit" & chr(13)
			' - After commit change the prompt
			nResult = objTab_L.Screen.WaitForStrings (vWaitForCommit, 60)
			Select Case nResult
				...
				Case 2 
					...
					Exit Do
				Case Else
					Call  TrDebug_No_Date ("Committing configuration on " & strSessionL, "OK", objDebug, MAX_LEN, 1, 1)   
			End Select	
			Call TrDebug_No_Date ("LOAD " & UCase(strSessionL) & " SUCCESS", "", objDebug, MAX_LEN, 3, 1)
			Exit Do
		Loop
		If bRollback Then 
			objTab_L.Screen.Send "rollback " & nRollBack & chr(13)
			objTab_L.Screen.WaitForString "@" & strHostL & "# "
			objTab_L.Screen.Send "commit" & chr(13)
			nResult = objTab_L.Screen.WaitForStrings (vWaitFors, 60)
			Select Case nResult
				Case 0 
					Call TrDebug("Restoring Old Configuration:", "TIME OUT" , objDebug, MAX_LEN, 1, 1)
					Exit Sub
				Case 3
					Call TrDebug("Restoring Old Configuration:" , "ROLLBACK OK" , objDebug, MAX_LEN, 1, 1)					    
			 End Select
		End If
        If bConnect Then 		
			objTab_L.Screen.WaitForString "@" & strHostL & "#"
